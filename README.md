...
between the ADHD and control groups via box plots. Every metric has a
plain-English explanation shown under the chart, plus a full glossary
expander.

Don't have the files yet? Click **"Load synthetic sample data instead"** to
populate the whole tab with randomly generated (clearly labeled) demo data,
shaped like the real dataset -- useful for trying the app immediately, or for
sharing a live link that works before a visitor uploads anything.

There's also a **Guess the Group** mini-game at the bottom -- two anonymized
box plots for a random metric, and you guess which one is the ADHD group.
...
channel distributions between groups. A "Load synthetic sample data instead"
button is also available here, so the tab is never empty.
...
The chat can also draw charts on request (e.g. "plot ASRS by group" or "show
diagnosis rate by sex") using a constrained tool -- it can only render
predefined chart types with real column names, not run arbitrary code. A row
of suggested-question buttons appears before you've sent your first message.

## Other features

- **Dark mode**: toggle in the sidebar. All charts re-theme using a validated
  dark-mode-safe palette, not just an inverted background.
- **Download buttons**: every tab lets you download the data behind its
  charts as CSV.
- **About section**: an expander at the top explains the whole app; it's
  expanded automatically on your first visit each session.
...
- Charts use a colorblind-safe categorical palette (validated per the
  project's dataviz guidelines), in both light and dark mode.
...
- Sample/demo data (in tabs 2 and 3, and in the "Guess the Group" game) is
  synthetic and randomly generated -- it is clearly labeled and is never
